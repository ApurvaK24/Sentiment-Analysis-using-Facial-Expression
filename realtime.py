...
import backend
from functions import find_target_size
...
def analysis(
    ...
    target_size = find_target_size(model_name=model_name)
    
    backend.build_model(model_name=model_name)
    print(f"facial recognition model {model_name} is just built")

    if enable_face_analysis:
        backend.build_model(model_name="Age")
        print("Age model is just built")
        backend.build_model(model_name="Gender")
        print("Gender model is just built")
        backend.build_model(model_name="Emotion")
    ...
    while True:
        ...
        if freeze == False:
            try:
                ...
                face_objs = backend.extract_faces(
                    img_path=img,
                    ...
                    for detected_face in detected_faces_final:
                                    ...
                                    for index, instance in emotion_df.iterrows():
                                        ...
                                        if x + w + pivot_img_size < resolution_x:
                                            ...
                                            if text_location_y < y + h:
                                                ...
                                                # cv2.putText(
                                                #     freeze_img,
                                                #     str(round(emotion_score, 2)),
                                                #     (x - pivot_img_size + 70 + bar_x, y + 13 + (index + 1) * 20),
                                                #     cv2.FONT_HERSHEY_SIMPLEX,
                                                #     0.5,
                                                #     (255, 255, 255),
                                                #     1,
                        ...
                        if len(dfs) > 0:
                            ...
                            if df.shape[0] > 0:
                                ...
                                source_objs = backend.extract_faces(
                                    img_path=label,
